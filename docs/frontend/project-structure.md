...
| Web scaffold (Kitajs HTML, HTMX, static) | **Done** | `app/src/web/mountWeb.ts`, `routes/<area>/page.tsx` + `handler.tsx` + `components/` + `tests/`, `public/` (HTMX via `vendor-htmx` in `app/package.json`). |
| Web route pattern | **Done** | Feature colocation; each area exports a **`Route`** from `handler.tsx` (e.g. `Home`, `SuspenseDemo`); `mapRoutes` in `web/routes/index.ts` (see `.cursor/skills/express-route-map`, `kitajs-html-web`). |
...
- HTMX `hx-get` routes return **component HTML** only (no `<html>`), from the same feature folder’s **`components/`** or an extra `router.get` in **`handler.tsx`**.
