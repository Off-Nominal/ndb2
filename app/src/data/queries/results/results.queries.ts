...
const getSeasonResultsLeaderboardIR: any = {"usedParamSet":{"season_id":true,"sort_by":true,"limit":true,"row_offset":true},"params":[{"name":"season_id","required":true,"transform":{"type":"scalar"},"locs":[{"a":130,"b":140},{"a":264,"b":274},{"a":399,"b":409},{"a":1016,"b":1026},{"a":1926,"b":1936},{"a":2653,"b":2663},{"a":3072,"b":3082}]},{"name":"sort_by","required":false,"transform":{"type":"scalar"},"locs":[{"a":5913,"b":5920},{"a":6025,"b":6032},{"a":6135,"b":6142},{"a":6239,"b":6246},{"a":6341,"b":6348},{"a":6441,"b":6448},{"a":6539,"b":6546},{"a":6643,"b":6650},{"a":6745,"b":6752},{"a":6853,"b":6860},{"a":6959,"b":6966},{"a":7065,"b":7072},{"a":7169,"b":7176},{"a":7267,"b":7274},{"a":7363,"b":7370},{"a":7453,"b":7460},{"a":7541,"b":7548},{"a":7633,"b":7640},{"a":7723,"b":7730},{"a":7815,"b":7822},{"a":7905,"b":7912},{"a":7997,"b":8004},{"a":8087,"b":8094},{"a":8173,"b":8180},{"a":8257,"b":8264},{"a":8341,"b":8348},{"a":8423,"b":8430},{"a":8525,"b":8532},{"a":8625,"b":8632},{"a":8721,"b":8728},{"a":8815,"b":8822},{"a":8909,"b":8916},{"a":9001,"b":9008},{"a":9097,"b":9104},{"a":9191,"b":9198},{"a":9291,"b":9298},{"a":9389,"b":9396},{"a":9477,"b":9484},{"a":9563,"b":9570},{"a":9660,"b":9667},{"a":9757,"b":9764},{"a":9878,"b":9885},{"a":9999,"b":10006},{"a":10106,"b":10113}]},{"name":"limit","required":true,"transform":{"type":"scalar"},"locs":[{"a":10222,"b":10228}]},{"name":"row_offset","required":true,"transform":{"type":"scalar"},"locs":[{"a":10237,"b":10248}]}],"statement":"WITH participants AS (\n  SELECT DISTINCT uid AS user_id FROM (\n    SELECT p.user_id AS uid FROM predictions p WHERE p.season_id = :season_id!\n    UNION\n    SELECT b.user_id FROM bets b\n    INNER JOIN predictions p ON p.id = b.prediction_id\n    WHERE p.season_id = :season_id!\n    UNION\n    SELECT v.user_id FROM votes v\n    INNER JOIN predictions p ON p.id = v.prediction_id\n    WHERE p.season_id = :season_id!\n  ) x\n),\npred_stats AS (\n  SELECT\n    p.user_id,\n    COUNT(*) FILTER (WHERE (p.status)::text = 'successful')::int AS predictions_successful,\n    COUNT(*) FILTER (WHERE (p.status)::text = 'failed')::int AS predictions_failed,\n    COUNT(*) FILTER (WHERE (p.status)::text = 'open')::int AS predictions_open,\n    COUNT(*) FILTER (WHERE (p.status)::text = 'closed')::int AS predictions_closed,\n    COUNT(*) FILTER (WHERE (p.status)::text = 'checking')::int AS predictions_checking,\n    COUNT(*) FILTER (WHERE (p.status)::text = 'retired')::int AS predictions_retired\n  FROM predictions p\n  WHERE p.season_id = :season_id!\n  GROUP BY p.user_id\n),\nbet_stats AS (\n  SELECT\n    b.user_id,\n    COUNT(*) FILTER (\n      WHERE (\n        ((p.status)::text = 'successful' AND b.endorsed IS TRUE) OR\n        ((p.status)::text = 'failed' AND b.endorsed IS FALSE)\n      ) AND b.valid IS TRUE\n    )::int AS bets_successful,\n    COUNT(*) FILTER (\n      WHERE (\n        ((p.status)::text = 'successful' AND b.endorsed IS FALSE) OR\n        ((p.status)::text = 'failed' AND b.endorsed IS TRUE)\n      ) AND b.valid IS TRUE\n    )::int AS bets_failed,\n    COUNT(*) FILTER (\n      WHERE ((p.status)::text IN ('open', 'closed', 'checking')) AND b.valid IS TRUE\n    )::int AS bets_pending,\n    COUNT(*) FILTER (WHERE (p.status)::text = 'retired' AND b.valid IS TRUE)::int AS bets_retired,\n    COUNT(*) FILTER (WHERE b.valid IS FALSE)::int AS bets_invalid\n  FROM bets b\n  INNER JOIN predictions p ON p.id = b.prediction_id\n  WHERE p.season_id = :season_id!\n  GROUP BY b.user_id\n),\nvote_stats AS (\n  SELECT\n    v.user_id,\n    COUNT(*) FILTER (WHERE v.vote IS TRUE)::int AS votes_yes,\n    COUNT(*) FILTER (WHERE v.vote IS FALSE)::int AS votes_no,\n    COUNT(*) FILTER (\n      WHERE ((p.status)::text = 'successful' AND v.vote IS TRUE)\n        OR ((p.status)::text = 'failed' AND v.vote IS FALSE)\n    )::int AS votes_affirmative,\n    COUNT(*) FILTER (\n      WHERE ((p.status)::text = 'successful' AND v.vote IS FALSE)\n        OR ((p.status)::text = 'failed' AND v.vote IS TRUE)\n    )::int AS votes_negative,\n    COUNT(*) FILTER (WHERE (p.status)::text = 'closed')::int AS votes_pending\n  FROM votes v\n  INNER JOIN predictions p ON p.id = v.prediction_id\n  WHERE p.season_id = :season_id!\n  GROUP BY v.user_id\n),\npoint_stats AS (\n  SELECT\n    b.user_id,\n    COALESCE(SUM(b.season_payout) FILTER (WHERE b.season_payout > 0), 0)::int AS points_rewards,\n    COALESCE(SUM(b.season_payout) FILTER (WHERE b.season_payout < 0), 0)::int AS points_penalties,\n    COALESCE(SUM(b.season_payout), 0)::int AS points_net\n  FROM bets b\n  INNER JOIN predictions p ON p.id = b.prediction_id\n  WHERE p.season_id = :season_id!\n  GROUP BY b.user_id\n),\njoined AS (\n  SELECT\n    pt.user_id,\n    u.discord_id,\n    COALESCE(pr.predictions_successful, 0) AS predictions_successful,\n    COALESCE(pr.predictions_failed, 0) AS predictions_failed,\n    COALESCE(pr.predictions_open, 0) AS predictions_open,\n    COALESCE(pr.predictions_closed, 0) AS predictions_closed,\n    COALESCE(pr.predictions_checking, 0) AS predictions_checking,\n    COALESCE(pr.predictions_retired, 0) AS predictions_retired,\n    COALESCE(be.bets_successful, 0) AS bets_successful,\n    COALESCE(be.bets_failed, 0) AS bets_failed,\n    COALESCE(be.bets_pending, 0) AS bets_pending,\n    COALESCE(be.bets_retired, 0) AS bets_retired,\n    COALESCE(be.bets_invalid, 0) AS bets_invalid,\n    COALESCE(vo.votes_yes, 0) AS votes_yes,\n    COALESCE(vo.votes_no, 0) AS votes_no,\n    COALESCE(vo.votes_affirmative, 0) AS votes_affirmative,\n    COALESCE(vo.votes_negative, 0) AS votes_negative,\n    COALESCE(vo.votes_pending, 0) AS votes_pending,\n    COALESCE(po.points_rewards, 0) AS points_rewards,\n    COALESCE(po.points_penalties, 0) AS points_penalties,\n    COALESCE(po.points_net, 0) AS points_net\n  FROM participants pt\n  INNER JOIN users u ON u.id = pt.user_id\n  LEFT JOIN pred_stats pr ON pr.user_id = pt.user_id\n  LEFT JOIN bet_stats be ON be.user_id = pt.user_id\n  LEFT JOIN vote_stats vo ON vo.user_id = pt.user_id\n  LEFT JOIN point_stats po ON po.user_id = pt.user_id\n),\nranked AS (\n  SELECT\n    j.user_id,\n    j.discord_id,\n    j.predictions_successful,\n    j.predictions_failed,\n    j.predictions_open,\n    j.predictions_closed,\n    j.predictions_checking,\n    j.predictions_retired,\n    j.bets_successful,\n    j.bets_failed,\n    j.bets_pending,\n    j.bets_retired,\n    j.bets_invalid,\n    j.votes_yes,\n    j.votes_no,\n    j.votes_affirmative,\n    j.votes_negative,\n    j.votes_pending,\n    j.points_rewards,\n    j.points_penalties,\n    j.points_net,\n    CAST((ROW_NUMBER() OVER (\n      ORDER BY j.points_net DESC, j.user_id ASC\n    )) AS integer) AS rank_points_net,\n    CAST((ROW_NUMBER() OVER (\n      ORDER BY j.predictions_successful DESC, j.user_id ASC\n    )) AS integer) AS rank_predictions_successful,\n    CAST((ROW_NUMBER() OVER (\n      ORDER BY j.bets_successful DESC, j.user_id ASC\n    )) AS integer) AS rank_bets_successful\n  FROM joined j\n)\nSELECT\n  r.user_id,\n  r.discord_id,\n  r.predictions_successful,\n  r.predictions_failed,\n  r.predictions_open,\n  r.predictions_closed,\n  r.predictions_checking,\n  r.predictions_retired,\n  r.bets_successful,\n  r.bets_failed,\n  r.bets_pending,\n  r.bets_retired,\n  r.bets_invalid,\n  r.votes_yes,\n  r.votes_no,\n  r.votes_affirmative,\n  r.votes_negative,\n  r.votes_pending,\n  r.points_rewards,\n  r.points_penalties,\n  r.points_net,\n  r.rank_points_net,\n  r.rank_predictions_successful,\n  r.rank_bets_successful\nFROM ranked r\nORDER BY\n  (CASE WHEN :sort_by::text = 'predictions_successful-desc' THEN r.predictions_successful END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_successful-asc' THEN r.predictions_successful END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_failed-desc' THEN r.predictions_failed END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_failed-asc' THEN r.predictions_failed END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_open-desc' THEN r.predictions_open END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_open-asc' THEN r.predictions_open END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_closed-desc' THEN r.predictions_closed END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_closed-asc' THEN r.predictions_closed END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_checking-desc' THEN r.predictions_checking END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_checking-asc' THEN r.predictions_checking END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_retired-desc' THEN r.predictions_retired END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_retired-asc' THEN r.predictions_retired END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_successful-desc' THEN r.bets_successful END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_successful-asc' THEN r.bets_successful END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_failed-desc' THEN r.bets_failed END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_failed-asc' THEN r.bets_failed END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_pending-desc' THEN r.bets_pending END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_pending-asc' THEN r.bets_pending END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_retired-desc' THEN r.bets_retired END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_retired-asc' THEN r.bets_retired END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_invalid-desc' THEN r.bets_invalid END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_invalid-asc' THEN r.bets_invalid END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_yes-desc' THEN r.votes_yes END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_yes-asc' THEN r.votes_yes END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_no-desc' THEN r.votes_no END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_no-asc' THEN r.votes_no END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_affirmative-desc' THEN r.votes_affirmative END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_affirmative-asc' THEN r.votes_affirmative END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_negative-desc' THEN r.votes_negative END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_negative-asc' THEN r.votes_negative END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_pending-desc' THEN r.votes_pending END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_pending-asc' THEN r.votes_pending END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'points_rewards-desc' THEN r.points_rewards END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'points_rewards-asc' THEN r.points_rewards END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'points_penalties-desc' THEN r.points_penalties END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'points_penalties-asc' THEN r.points_penalties END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'points_net-desc' THEN r.points_net END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'points_net-asc' THEN r.points_net END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'rank_points_net-desc' THEN r.rank_points_net END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'rank_points_net-asc' THEN r.rank_points_net END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'rank_predictions_successful-desc' THEN r.rank_predictions_successful END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'rank_predictions_successful-asc' THEN r.rank_predictions_successful END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'rank_bets_successful-desc' THEN r.rank_bets_successful END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'rank_bets_successful-asc' THEN r.rank_bets_successful END) DESC NULLS LAST,\n  r.user_id ASC\nLIMIT :limit!\nOFFSET :row_offset!"};
...
const getAllTimeResultsLeaderboardIR: any = {"usedParamSet":{"sort_by":true,"limit":true,"row_offset":true},"params":[{"name":"sort_by","required":false,"transform":{"type":"scalar"},"locs":[{"a":5463,"b":5470},{"a":5575,"b":5582},{"a":5685,"b":5692},{"a":5789,"b":5796},{"a":5891,"b":5898},{"a":5991,"b":5998},{"a":6089,"b":6096},{"a":6193,"b":6200},{"a":6295,"b":6302},{"a":6403,"b":6410},{"a":6509,"b":6516},{"a":6615,"b":6622},{"a":6719,"b":6726},{"a":6817,"b":6824},{"a":6913,"b":6920},{"a":7003,"b":7010},{"a":7091,"b":7098},{"a":7183,"b":7190},{"a":7273,"b":7280},{"a":7365,"b":7372},{"a":7455,"b":7462},{"a":7547,"b":7554},{"a":7637,"b":7644},{"a":7723,"b":7730},{"a":7807,"b":7814},{"a":7891,"b":7898},{"a":7973,"b":7980},{"a":8075,"b":8082},{"a":8175,"b":8182},{"a":8271,"b":8278},{"a":8365,"b":8372},{"a":8459,"b":8466},{"a":8551,"b":8558},{"a":8647,"b":8654},{"a":8741,"b":8748},{"a":8841,"b":8848},{"a":8939,"b":8946},{"a":9027,"b":9034},{"a":9113,"b":9120},{"a":9210,"b":9217},{"a":9307,"b":9314},{"a":9428,"b":9435},{"a":9549,"b":9556},{"a":9656,"b":9663}]},{"name":"limit","required":true,"transform":{"type":"scalar"},"locs":[{"a":9772,"b":9778}]},{"name":"row_offset","required":true,"transform":{"type":"scalar"},"locs":[{"a":9787,"b":9798}]}],"statement":"WITH participants AS (\n  SELECT DISTINCT uid AS user_id FROM (\n    SELECT user_id AS uid FROM predictions\n    UNION\n    SELECT user_id FROM bets\n    UNION\n    SELECT user_id FROM votes\n  ) x\n),\npred_stats AS (\n  SELECT\n    p.user_id,\n    COUNT(*) FILTER (WHERE (p.status)::text = 'successful')::int AS predictions_successful,\n    COUNT(*) FILTER (WHERE (p.status)::text = 'failed')::int AS predictions_failed,\n    COUNT(*) FILTER (WHERE (p.status)::text = 'open')::int AS predictions_open,\n    COUNT(*) FILTER (WHERE (p.status)::text = 'closed')::int AS predictions_closed,\n    COUNT(*) FILTER (WHERE (p.status)::text = 'checking')::int AS predictions_checking,\n    COUNT(*) FILTER (WHERE (p.status)::text = 'retired')::int AS predictions_retired\n  FROM predictions p\n  GROUP BY p.user_id\n),\nbet_stats AS (\n  SELECT\n    b.user_id,\n    COUNT(*) FILTER (\n      WHERE (\n        ((p.status)::text = 'successful' AND b.endorsed IS TRUE) OR\n        ((p.status)::text = 'failed' AND b.endorsed IS FALSE)\n      ) AND b.valid IS TRUE\n    )::int AS bets_successful,\n    COUNT(*) FILTER (\n      WHERE (\n        ((p.status)::text = 'successful' AND b.endorsed IS FALSE) OR\n        ((p.status)::text = 'failed' AND b.endorsed IS TRUE)\n      ) AND b.valid IS TRUE\n    )::int AS bets_failed,\n    COUNT(*) FILTER (\n      WHERE ((p.status)::text IN ('open', 'closed', 'checking')) AND b.valid IS TRUE\n    )::int AS bets_pending,\n    COUNT(*) FILTER (WHERE (p.status)::text = 'retired' AND b.valid IS TRUE)::int AS bets_retired,\n    COUNT(*) FILTER (WHERE b.valid IS FALSE)::int AS bets_invalid\n  FROM bets b\n  INNER JOIN predictions p ON p.id = b.prediction_id\n  GROUP BY b.user_id\n),\nvote_stats AS (\n  SELECT\n    v.user_id,\n    COUNT(*) FILTER (WHERE v.vote IS TRUE)::int AS votes_yes,\n    COUNT(*) FILTER (WHERE v.vote IS FALSE)::int AS votes_no,\n    COUNT(*) FILTER (\n      WHERE ((p.status)::text = 'successful' AND v.vote IS TRUE)\n        OR ((p.status)::text = 'failed' AND v.vote IS FALSE)\n    )::int AS votes_affirmative,\n    COUNT(*) FILTER (\n      WHERE ((p.status)::text = 'successful' AND v.vote IS FALSE)\n        OR ((p.status)::text = 'failed' AND v.vote IS TRUE)\n    )::int AS votes_negative,\n    COUNT(*) FILTER (WHERE (p.status)::text = 'closed')::int AS votes_pending\n  FROM votes v\n  INNER JOIN predictions p ON p.id = v.prediction_id\n  GROUP BY v.user_id\n),\npoint_stats AS (\n  SELECT\n    b.user_id,\n    COALESCE(SUM(b.payout) FILTER (WHERE b.payout > 0), 0)::int AS points_rewards,\n    COALESCE(SUM(b.payout) FILTER (WHERE b.payout < 0), 0)::int AS points_penalties,\n    COALESCE(SUM(b.payout), 0)::int AS points_net\n  FROM bets b\n  GROUP BY b.user_id\n),\njoined AS (\n  SELECT\n    pt.user_id,\n    u.discord_id,\n    COALESCE(pr.predictions_successful, 0) AS predictions_successful,\n    COALESCE(pr.predictions_failed, 0) AS predictions_failed,\n    COALESCE(pr.predictions_open, 0) AS predictions_open,\n    COALESCE(pr.predictions_closed, 0) AS predictions_closed,\n    COALESCE(pr.predictions_checking, 0) AS predictions_checking,\n    COALESCE(pr.predictions_retired, 0) AS predictions_retired,\n    COALESCE(be.bets_successful, 0) AS bets_successful,\n    COALESCE(be.bets_failed, 0) AS bets_failed,\n    COALESCE(be.bets_pending, 0) AS bets_pending,\n    COALESCE(be.bets_retired, 0) AS bets_retired,\n    COALESCE(be.bets_invalid, 0) AS bets_invalid,\n    COALESCE(vo.votes_yes, 0) AS votes_yes,\n    COALESCE(vo.votes_no, 0) AS votes_no,\n    COALESCE(vo.votes_affirmative, 0) AS votes_affirmative,\n    COALESCE(vo.votes_negative, 0) AS votes_negative,\n    COALESCE(vo.votes_pending, 0) AS votes_pending,\n    COALESCE(po.points_rewards, 0) AS points_rewards,\n    COALESCE(po.points_penalties, 0) AS points_penalties,\n    COALESCE(po.points_net, 0) AS points_net\n  FROM participants pt\n  INNER JOIN users u ON u.id = pt.user_id\n  LEFT JOIN pred_stats pr ON pr.user_id = pt.user_id\n  LEFT JOIN bet_stats be ON be.user_id = pt.user_id\n  LEFT JOIN vote_stats vo ON vo.user_id = pt.user_id\n  LEFT JOIN point_stats po ON po.user_id = pt.user_id\n),\nranked AS (\n  SELECT\n    j.user_id,\n    j.discord_id,\n    j.predictions_successful,\n    j.predictions_failed,\n    j.predictions_open,\n    j.predictions_closed,\n    j.predictions_checking,\n    j.predictions_retired,\n    j.bets_successful,\n    j.bets_failed,\n    j.bets_pending,\n    j.bets_retired,\n    j.bets_invalid,\n    j.votes_yes,\n    j.votes_no,\n    j.votes_affirmative,\n    j.votes_negative,\n    j.votes_pending,\n    j.points_rewards,\n    j.points_penalties,\n    j.points_net,\n    CAST((ROW_NUMBER() OVER (\n      ORDER BY j.points_net DESC, j.user_id ASC\n    )) AS integer) AS rank_points_net,\n    CAST((ROW_NUMBER() OVER (\n      ORDER BY j.predictions_successful DESC, j.user_id ASC\n    )) AS integer) AS rank_predictions_successful,\n    CAST((ROW_NUMBER() OVER (\n      ORDER BY j.bets_successful DESC, j.user_id ASC\n    )) AS integer) AS rank_bets_successful\n  FROM joined j\n)\nSELECT\n  r.user_id,\n  r.discord_id,\n  r.predictions_successful,\n  r.predictions_failed,\n  r.predictions_open,\n  r.predictions_closed,\n  r.predictions_checking,\n  r.predictions_retired,\n  r.bets_successful,\n  r.bets_failed,\n  r.bets_pending,\n  r.bets_retired,\n  r.bets_invalid,\n  r.votes_yes,\n  r.votes_no,\n  r.votes_affirmative,\n  r.votes_negative,\n  r.votes_pending,\n  r.points_rewards,\n  r.points_penalties,\n  r.points_net,\n  r.rank_points_net,\n  r.rank_predictions_successful,\n  r.rank_bets_successful\nFROM ranked r\nORDER BY\n  (CASE WHEN :sort_by::text = 'predictions_successful-desc' THEN r.predictions_successful END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_successful-asc' THEN r.predictions_successful END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_failed-desc' THEN r.predictions_failed END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_failed-asc' THEN r.predictions_failed END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_open-desc' THEN r.predictions_open END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_open-asc' THEN r.predictions_open END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_closed-desc' THEN r.predictions_closed END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_closed-asc' THEN r.predictions_closed END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_checking-desc' THEN r.predictions_checking END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_checking-asc' THEN r.predictions_checking END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_retired-desc' THEN r.predictions_retired END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'predictions_retired-asc' THEN r.predictions_retired END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_successful-desc' THEN r.bets_successful END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_successful-asc' THEN r.bets_successful END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_failed-desc' THEN r.bets_failed END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_failed-asc' THEN r.bets_failed END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_pending-desc' THEN r.bets_pending END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_pending-asc' THEN r.bets_pending END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_retired-desc' THEN r.bets_retired END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_retired-asc' THEN r.bets_retired END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_invalid-desc' THEN r.bets_invalid END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'bets_invalid-asc' THEN r.bets_invalid END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_yes-desc' THEN r.votes_yes END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_yes-asc' THEN r.votes_yes END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_no-desc' THEN r.votes_no END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_no-asc' THEN r.votes_no END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_affirmative-desc' THEN r.votes_affirmative END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_affirmative-asc' THEN r.votes_affirmative END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_negative-desc' THEN r.votes_negative END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_negative-asc' THEN r.votes_negative END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_pending-desc' THEN r.votes_pending END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'votes_pending-asc' THEN r.votes_pending END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'points_rewards-desc' THEN r.points_rewards END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'points_rewards-asc' THEN r.points_rewards END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'points_penalties-desc' THEN r.points_penalties END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'points_penalties-asc' THEN r.points_penalties END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'points_net-desc' THEN r.points_net END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'points_net-asc' THEN r.points_net END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'rank_points_net-desc' THEN r.rank_points_net END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'rank_points_net-asc' THEN r.rank_points_net END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'rank_predictions_successful-desc' THEN r.rank_predictions_successful END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'rank_predictions_successful-asc' THEN r.rank_predictions_successful END) DESC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'rank_bets_successful-desc' THEN r.rank_bets_successful END) ASC NULLS LAST,\n  (CASE WHEN :sort_by::text = 'rank_bets_successful-asc' THEN r.rank_bets_successful END) DESC NULLS LAST,\n  r.user_id ASC\nLIMIT :limit!\nOFFSET :row_offset!"};
