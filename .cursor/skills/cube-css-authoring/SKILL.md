...
- **BEM `__element` in markup** is reserved for **rare** cases: a **stable named** hook (multiple elements of the same kind, **variants** shared with client JS, or a hook you truly cannot express with **`&`** and structure). It is **not** the default way to name every inner node. **Do not** rely on class strings in unit tests (see **`kitajs-html-web`** — use semantics / **`data-*`** instead). If you have a **single** form, `& > form` nested under `.site-nav` is enough; you may **omit** a `site-nav__logout` class.
