...
- **`body`** + **`[ glass-background ]`** (see **`utilities.css`**) is the **base** display glass: gradient, grid, accent radials, **opaque** diagonal (no frosted `color-surface` in that stack). **Below `breakpoint.wide` (`80rem`)** the authenticated right nav often reuses the same **`.glass-background`** (second sheet on top, same skin). From **wide** upward, the **nav column** is typically not a second stack—**transparent**, side by side on the single body layer. Exact widths and CSS live in **`page-layout`** / **`site-nav`**; tier names and `rem` cut points are authoritative in **`web-breakpoints`**.
