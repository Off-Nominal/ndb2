...
- **Compile output:** `pnpm run build` runs **`prepare-dev`** (Wireit: **`vendor-htmx`**, **`build:tokens`**, **`build:css`**, **`build:client-js`**, **`transfer-web`**) then `tsc`, `tsc-alias`, `transfer-queries`. **`build:css`** emits **`dist/web/public/cube.css`**; hand-placed static (fonts, etc.) in **`app/src/web/public/`** is merged by **`transfer-web`**.
- **Dev:** **`tsx watch`** + Vite middleware. **`HtmlHead`** uses **`cubeStylesheetHref()`**: **`/src/web/styles/cube-entry.css`** (Vite **CSS HMR**). **`dev-watch-assets`** on token JSON runs **`build:tokens`** + **`transfer-web`**; on other CSS it runs **`generate-cube-blocks-manifest.mjs`** + **`transfer-web`** (ignores **`src/web/generated/**`** so manifest writes do not loop). Client **`*.client.*`** still runs **`build:client-js`** and touches **`index.ts`**.
