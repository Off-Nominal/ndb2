...
- **Dev:** **`tsx watch`** + Vite middleware. **`HtmlHead`** uses **`cubeStylesheetHref()`**: **`/src/web/styles/cube-entry.css`** (Vite **CSS HMR**). **`dev-watch-assets`** on token JSON runs **`build:tokens`** + **`build:web-assets`**; on other CSS it runs **`generate-cube-blocks-manifest.mjs`** + **`build:web-assets`** (ignores **`src/web/generated/**`** so manifest writes do not loop). Client **`*.client.*`** still runs **`build:client-js`** and touches **`index.ts`**.
