...
description: >-
  ...
  changing queries under api/src/v2/queries, wiring PoolClient, transactions,
...
- **SQL and generated code**: `api/src/v2/queries/<entity>/`
...
- **Config**: `api/pgtyped.config.json` — `srcDir` is `./src/v2/queries`; each `*.sql` file emits a sibling **`{{name}}.queries.ts`** (e.g. `bets.sql` → `bets.queries.ts`).
...
1. Add `api/src/v2/queries/<entity>/<entity>.sql` with `@name` queries.
