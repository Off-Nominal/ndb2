...
description: >-
  ...
  changing queries under api/src/data/queries, wiring PoolClient, transactions,
...
- **SQL and generated code**: `api/src/data/queries/<entity>/`
...
- **Config**: `api/pgtyped.config.json` — `srcDir` is `./src/data/queries`; each `*.sql` file emits a sibling **`{{name}}.queries.ts`** (e.g. `bets.sql` → `bets.queries.ts`).
...
1. Add `api/src/data/queries/<entity>/<entity>.sql` with `@name` queries.
