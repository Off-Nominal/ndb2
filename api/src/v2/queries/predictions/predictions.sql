...
/*
  Single round-trip for GET prediction by id: one client.query, no overlapping
  queries (pg PoolClient is single-flight). Subqueries mirror getBetsByPredictionId,
  getVotesByPredictionId, and getSnoozeChecksByPredictionId.
*/
/* @name getPredictionDetailById */
WITH
  pred AS (
    SELECT
      p.id,
